...
- 为了保证图可读，不要把文字直接放在线上；需要解释连接关系时，用独立文本框放在线旁边，必要时加白色底色。
- 避免连接线共用同一段路径；反馈线尽量走主流程上方，数据读取/写回线尽量走数据节点下方。
- 把导出的 PNG 当作视觉检查结果：检查文字、线条、节点是否重叠，再用 `-UseActiveDocument` 在当前页面继续重画。
- 脚本不会运行 Visio 宏。
